...
export class SelectionUtilies {
	 ...
	 */
	static doesSelectionHaveTextMark({ from, to }: Selection, doc: Node, name: string): boolean {
		if (from === to) {
			return false;
		}
	 ...
	 */
	static findTextMarks({ from, to }: Selection, doc: Node): TextMarkPresence {
	...
	static getBlockAttributeRatio<Value>(
		...
		doc.nodesBetween(from, to, (node) => {
			...
			if (
				...
				(typeof value === 'function' && value(val))
			) {
				occurrences++;
			}
		});
